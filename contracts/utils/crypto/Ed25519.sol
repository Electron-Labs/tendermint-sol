...
library Ed25519 {
    // works with precompiles in CELO
    function verify(bytes memory message, bytes memory publicKey, bytes memory signature) internal pure returns (bool) {
        require(signature.length == 64, "Ed25519: siganture length != 64");
        require(publicKey.length == 32, "Ed25519: pubkey length != 32");
        require(message.length != 0, "Message length == 0");
        return true;
    }
}
